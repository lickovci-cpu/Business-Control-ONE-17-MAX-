...
await assert.rejects(()=>control.completeTask(task.id,[{type:'message_sent',referenceId:'msg-1',taskId:task.id,action:task.action,payloadHash:task.payloadHash,attemptId:task.attemptId,resultHash:fingerprint({id:'msg-1'})}],req,'jihoceske',{id:'different'}),/EVIDENCE_RESULT_MISMATCH/);
const result={id:'msg-1',status:'sent'};
const done=await control.completeTask(task.id,[{type:'message_sent',referenceId:'msg-1',taskId:task.id,action:task.action,payloadHash:task.payloadHash,attemptId:task.attemptId,resultHash:fingerprint(result)}],req,'jihoceske',result);
...
assert.equal(done.evidence[0].attemptId,task.attemptId);
