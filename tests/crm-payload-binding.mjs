...
const hash=(v)=>{
  ...
  return (await import('node:crypto')).createHash('sha256').update(stable(v)).digest('hex');
};

const payload=(p={})=>({...base,...p});
const taskFor=(action,p)=>({id:'task-1',project:'jihoceske',action:`crm:lead-${action}`,payloadHash:null});
...
for(const [action,p] of [
  ...
  const t=taskFor(action,p);t.payloadHash=await hash(p);
  ...
  if(action==='update'){changed.id='lead-2';assert.throws(()=>validateApprovedPayload(t,t.id,action,changed,'jihoceske'),/PAYLOAD_MISMATCH|PROJECT_MISMATCH/);changed.id='lead-1';changed.patch.note='CHANGED';}
...
const exact=payload({id:'lead-1',patch:{estimated_value:10000,note:'TEST'}});const exactTask=taskFor('update',exact);exactTask.payloadHash=await hash(exact);
...
// Replay is enforced by Control Plane consumeApproval; keep an explicit regression marker here.
