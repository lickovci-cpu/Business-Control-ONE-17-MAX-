...
import {body,projectKey,sendError,safeEqual,supabaseBaseUrl,supabaseServiceKey} from '../_lib.js';
...
async function ingestOrder(project,payload,organizationId,event){
  ...
  const normalizedItems=(await Promise.all(items.map(async(item,index)=>{
    ...
    const rawProductId=clean(item?.product_id,80);
    const sku=clean(item?.sku,100)||null;
    let catalog=null;
    if(UUID_RE.test(rawProductId)){
      const rows=await sb(`merch_products?organization_id=eq.${organizationId}&id=eq.${rawProductId}&active=eq.true&select=id,sku,name,price&limit=1`);
      catalog=rows?.[0]||null;
    }else if(sku){
      const rows=await sb(`merch_products?organization_id=eq.${organizationId}&sku=eq.${encodeURIComponent(sku)}&active=eq.true&select=id,sku,name,price&limit=1`);
      catalog=rows?.[0]||null;
    }
    const catalogPrice=Number(catalog?.price);
    const verifiedCatalogPrice=Number.isFinite(catalogPrice)&&catalogPrice>=0;
    const resolvedProductId=catalog?.id||null;
    const effectivePrice=resolvedProductId&&verifiedCatalogPrice&&event?.eventType==='checkout.created' ? catalogPrice : unitPrice;
    return {
      product_id:resolvedProductId,
      product_name:clean(item?.product_name||catalog?.name||item?.name||'Merch',300),
      sku:sku||clean(catalog?.sku,100)||null,
      variant:(item?.variant&&typeof item.variant==='object')?item.variant:{},
      quantity,
      unit_price:effectivePrice,
      line_total:quantity*effectivePrice,
      price_verification:resolvedProductId ? (verifiedCatalogPrice && Math.abs(unitPrice-catalogPrice)<0.01 ? 'verified_catalog' : 'catalog_linked') : 'not_cataloged'
    };
  })));
  ...
  const priceVerification=normalizedItems.every(item=>item.price_verification==='verified_catalog')?'verified_catalog':normalizedItems.some(item=>item.price_verification==='catalog_linked')?'catalog_linked':'not_cataloged';
