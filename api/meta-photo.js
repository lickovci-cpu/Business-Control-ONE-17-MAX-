...
export default async function handler(req,res){
  ...
  try{
    ...
    verifyConfirmation(req.headers['x-confirm-token'],'meta:photo-upload',{project,photos:manifest});
    ...
    const id=await uploadToMeta(buf,mime,index,await resolveMeta(project)),proofPayload={project,index,id,sha256},proof=createConfirmation('meta:photo-proof',proofPayload,900);return res.json({id,proof});
