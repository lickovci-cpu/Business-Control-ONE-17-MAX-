...
export default async function handler(req,res){
  ...
  try{
    ...
    if(action==='preview-send'){const payload=normalize(b);validate(payload);return res.json({preview:payload,confirmationToken:createConfirmation('comms:send',payload)});}
    if(action==='send'){const payload=normalize(b);validate(payload);verifyConfirmation(b.confirmationToken,'comms:send',payload);const result=await sendCommunication(payload);return res.json({ok:true,result});}
    if(action==='preview-queue'){const when=new Date(b.when);if(!b.when||Number.isNaN(when.getTime()))return res.status(400).json({error:'INVALID_TIME'});const payload=normalize({...b,when:when.toISOString()},{withWhen:true});validate(payload,{withWhen:true});return res.json({preview:payload,confirmationToken:createConfirmation('comms:queue',payload)});}
    if(action==='queue'){
      const when=new Date(b.when);if(!b.when||Number.isNaN(when.getTime()))return res.status(400).json({error:'INVALID_TIME'});const payload=normalize({...b,when:when.toISOString()},{withWhen:true});validate(payload,{withWhen:true});verifyConfirmation(b.confirmationToken,'comms:queue',payload);
      const item={id:randomUUID(),...payload,status:'queued',attempts:0,nextAttemptAt:payload.when,approvedAt:new Date().toISOString(),createdAt:new Date().toISOString()};await saveItem(item);await kvLpush(INDEX,item.id);await kvLtrim(INDEX,0,499);await kvZadd(DUE,new Date(item.when).getTime(),item.id);return res.json({ok:true,item});
    }
    if(action==='cancel'){const x=await getItem(String(b.id||''));if(!x)return res.status(404).json({error:'NOT_FOUND'});sameProject(x,b.project);if(!['queued','retry_wait'].includes(x.status))return res.status(409).json({error:'NOT_QUEUED'});x.status='cancelled';x.cancelledAt=new Date().toISOString();await saveItem(x);await kvZrem(DUE,x.id);return res.json({ok:true,item:x});}
    if(action==='preview-retry'){const x=await getItem(String(b.id||''));if(!x)return res.status(404).json({error:'NOT_FOUND'});sameProject(x,b.project);if(!['failed','needs_review'].includes(x.status))return res.status(409).json({error:'NOT_RETRYABLE'});const payload={project:x.project,id:x.id,status:x.status,to:x.to,channel:x.channel,text:x.text,subject:x.subject||''};return res.json({preview:payload,confirmationToken:createConfirmation('comms:retry',payload)});}
    if(action==='retry'){const x=await getItem(String(b.id||''));if(!x)return res.status(404).json({error:'NOT_FOUND'});sameProject(x,b.project);const payload={project:x.project,id:x.id,status:x.status,to:x.to,channel:x.channel,text:x.text,subject:x.subject||''};verifyConfirmation(b.confirmationToken,'comms:retry',payload);x.status='queued';x.nextAttemptAt=new Date().toISOString();x.lastError='';x.deliveryUncertain=false;await saveItem(x);await kvZadd(DUE,Date.now(),x.id);return res.json({ok:true,item:x});}
